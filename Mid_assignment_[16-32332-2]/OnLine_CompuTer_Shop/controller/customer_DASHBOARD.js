...
//>>>>>>>>>>>>>>get,post for TD_Available_Regions>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
				...
				router.post('/TD_Available_Regions', (req, res) =>
				{
					var word = req.body.search;
					rModel.search(word, (results) =>
					{
						console.log("in controller " + { td: results });
						res.render('', { td: results });

					});

				});

				router.get('/TD_Available_Regions/ajaxsearch/:td_id', (req,res) =>
				{
					var txt = req.body.search;
					console.log(txt);

					var word = req.params.td_id;
					console.log('word value '+word);
							...
							if(word!=null || word!=undefined || txt!=null)
							{
										rModel.search(word, (results) =>
										{
												...
												{
														str+='<a style="position:relative; left:30px; font-size:20px; margin-top:50px;" href="/TD_Available_Regions/'+results[i].td_id+'">'+results[i].r_name+'</a><br><br>';
												}
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//>>>>>>>>>>>>>>get,post for TD_Traveled_Regions>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

				router.get('/TD_Traveled_Regions', (req, res) =>
				{
						travModel.getAll(function(result)
						{
								res.render('td_DASHBOARD/TD_Traveled_Regions', { td : result, username: req.session.username});
						});

				});

				router.get('/TD_Traveled_Regions/insert', (req, res) =>
				{
						res.render('td_DASHBOARD/TD_Trav_insert');
				});

				router.post('/TD_Traveled_Regions/insert', (req, res) =>
				{
									var td =	{
									t_name	 : req.body.t_name,
									v_place		:	req.body.v_place,
									...
									date				: req.body.date
										...
										travModel.insert(td, (status) =>
										{
													if(status)
													{
														res.redirect('/td_DASHBOARD/TD_Traveled_Regions');
													}
													else
													{
														res.redirect('/td_DASHBOARD/TD_Trav_insert');
													}
												});

						});

				router.get('/TD_Traveled_Regions/edit/:td_id', (req, res) =>
				{
						travModel.get(req.params.td_id, (result) =>
						{
								res.render('td_DASHBOARD/TD_Trav_update', {td: result});
						});
				});

				router.post('/TD_Traveled_Regions/edit/:td_id', (req, res) =>
				{
						var td =	{
							t_name	 : req.body.t_name,
							v_place		:	req.body.v_place,
				...
				router.get('/TD_Traveled_Regions/delete/:td_id', (req, res) =>
				{
						travModel.get(req.params.td_id, (result) =>
						{
								res.render('td_DASHBOARD/TD_Trav_delete', {td: result});
						});

				});

				router.post('/TD_Traveled_Regions/delete/:td_id', (req, res) =>
				{
							travModel.delete(req.body.td_id, (status) =>
							{
										if(status)
										{
											res.redirect('/td_DASHBOARD/TD_Traveled_Regions');
